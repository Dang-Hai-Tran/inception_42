...
while ! mariadb -h${DB_HOST} -u${WP_DB_USER} -p${WP_DB_PWD} ${WP_DB_NAME} &>/dev/null; do
    sleep 3
done
...
if [ -f ${WP_PATH}/wp-config.php ]; then
    ...
    wp-cli.phar config create --dbname=${WP_DB_NAME} --dbuser=${WP_DB_USER} --dbpass=${WP_DB_PWD} --dbhost=${DB_HOST} --path=${WP_PATH} --allow-root
    echo "[WP config] Installing WordPress core..."
    wp-cli.phar core install --url=${NGINX_HOST}/wordpress --title=${WP_TITLE} --admin_user=${WP_ADMIN} --admin_password=${WP_ADMIN_PWD} --admin_email=${WP_ADMIN_EMAIL} --path=${WP_PATH} --allow-root
    echo "[WP config] Creating WordPress default user..."
    wp-cli.phar user create $WP_USER ${WP_USER_EMAIL} --user_pass=${WP_USER_PWD} --role=subscriber --display_name=${WP_USER} --porcelain --path=${WP_PATH} --allow-root
