...
then
    ...
    echo "ALTER USER 'root'@'localhost' IDENTIFIED BY '${DB_ROOT_PWD}';" >> ${TMP}
    echo "CREATE DATABASE ${WP_DB_NAME};" >> ${TMP}
    echo "CREATE USER '${WP_DB_USER} '@'%' IDENTIFIED BY '${WP_DB_PWD}';" >> ${TMP}
    echo "GRANT ALL PRIVILEGES ON ${WP_DB_NAME}.* TO '${WP_DB_USER}'@'%' IDENTIFIED BY '${WP_DB_PWD}';" >> ${TMP}
    echo "FLUSH PRIVILEGES;" >> ${TMP}
